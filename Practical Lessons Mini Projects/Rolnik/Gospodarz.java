...
public class Gospodarz extends Rolnik{
	...
	private void symulujTure(Ogrod ogrod) {
		int liczbaGrzadek = ogrod.dajLiczbeGrzadek();
		for (int grzadka = 1; grzadka <= liczbaGrzadek; grzadka++) {
			if (ogrod.czyGrzadkaPusta(grzadka)) {
				zasadz(grzadka, ogrod);
			}
			if (czyPowinienZebrac(grzadka, ogrod)) {
				zbierzOrazPosadz(grzadka, ogrod);
			}
			aktualizujCene(grzadka, ogrod);
		}
	}
	private boolean czyPowinienZebrac(int grzadka, Ogrod ogrod) {
		//System.out.println("PATRZE " + ogrod.cenaTerazWarzywa(grzadka, dniOdZasadzenia.get(grzadka)+1) + " " + cenyWarzyw.get(grzadka) + " " + (dniOdZasadzenia.get(grzadka)+1));
		return (ogrod.cenaTerazWarzywa(grzadka, dniOdZasadzenia.get(grzadka)+1) <
		cenyWarzyw.get(grzadka));
	}
	@Override
	public void zbierzOrazPosadz(int grzadka, Ogrod ogrod) {
		System.out.print("ZEBRALEM: ");
		ogrod.wypiszNazweWarzywa(grzadka);
		System.out.print(" (WARTOSC: ");
		System.out.print(ogrod.cenaTerazWarzywa(grzadka,
		dniOdZasadzenia.get(grzadka)+1));
		System.out.println(" zl ) !");
		zarobek += ogrod.cenaTerazWarzywa(grzadka, dniOdZasadzenia.get(grzadka)+1);
		zasadz(grzadka, ogrod);
	}
	@Override
